<template>
  ...
  <div class="tab" v-if="Object.keys(obj).lengdiv != 0">
    ...
    <div
      v-for="(item, index) in obj.hotSongs"
      :key="index"
      class="geq"
      @mouseover="ff(index)"
      @mouseout="ff2()"
...
export default {
  ...
  computed: {
    fa() {
      return function (index) {
        if (typeof this.obj.hotSongs[index].alia[0] != "undefined") {
          return " " + "-(" + this.obj.hotSongs[index].alia[0] + ")";
        }
      };
    },
  },
  methods: {
    ...
    //歌手名
    m1(index) {
      return this.obj.hotSongs[index].ar[0].name;
    },
    m2(index) {
      if (this.obj.hotSongs[index].ar.length >= 2) {
        return "/" + this.obj.hotSongs[index].ar[1].name;
      }
      return "";
    },
    fn(index) {
    ...
    aoudi(id, index, gqname, img, zjname) {
      ...
      this.$store.commit("setarr", this.obj.hotSongs);
      this.$store.commit("setzj", zjname);
      this.$store.commit("setjzdk", false);
    },
    aoudi2() {
      this.$store.dispatch("seturl", this.obj.hotSongs[0].id);
      let name = this.m1(0) + this.m2(0);
      this.$store.commit("setname", name);
      this.$store.commit("setgqname", this.obj.hotSongs[0].name);
      this.$store.commit("setimga", this.obj.hotSongs[0].al.picUrl);
      this.$store.commit("setindex", 0);
      this.$store.commit("setarr", this.obj.hotSongs);
      this.$store.commit("setzj", this.obj.hotSongs[0].al.name);
      this.$store.commit("setjzdk", false);
    },
  },
  props: {
